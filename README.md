...
| `tochka_check_invoices` | Проверить все ожидающие счета через выписку, оплаченные удаляются автоматически |

Данные хранятся в `~/.config/mcp-server-tochka-bank/pending_invoices.json`.

После выставления счёта Claude сам предложит добавить его в отслеживание. При старте сессии — покажет неоплаченные и предложит запустить периодическую проверку.
