    ...
    "Transaction": [
        {
            ...
            "description": "Перечисл. ден.ср-в по дог. с ООО \"ТЕСТПЭЙ\" за пер.",
            "documentNumber": "101",
            "DebtorParty": {"inn": "770000000010", "name": "ПАО «Тестбанк»"},
        ...
        },
        {
            ...
            "description": "Перечисл. ден.ср-в по дог. с ООО \"ТЕСТПЭЙ\" за пер.",
            "documentNumber": "102",
            "DebtorParty": {"inn": "770000000020", "name": "НКО «Тестрасчёты» (АО)"},
        ...
        },
        {
            ...
            "description": "Оплата по счёту №42 от покупателя",
            "documentNumber": "103",
            "DebtorParty": {"inn": "770000000020", "name": "НКО «Тестрасчёты» (АО)"},
            "DebtorAccount": {"identification": "40702810200000000020"},
            "DebtorAgent": {"identification": "044525020"},
        ...
        },
        {
            ...
            "CreditorParty": {"inn": "770000000099", "name": "ООО Поставщик"},
...
async def test_tochka_incoming_all():
    ...
    with patch("mcp_server_tochka_bank.server.TochkaAPI") as MockAPI:
        ...
        async with create_connected_server_and_client_session(mcp._mcp_server) as session:
            ...
            assert data["by_inn"]["770000000010"]["amount"] == 2917.33
            assert data["by_inn"]["770000000010"]["count"] == 1
            assert data["by_inn"]["770000000020"]["amount"] == 33641.45
    ...
    with patch("mcp_server_tochka_bank.server.TochkaAPI") as MockAPI:
        ...
        async with create_connected_server_and_client_session(mcp._mcp_server) as session:
            result = await session.call_tool("tochka_incoming", {
                "month": 3, "year": 2026, "inn": "770000000020",
            })
            assert not result.isError
            data = json.loads(result.content[0].text)
...
@pytest.mark.anyio
async def test_tochka_incoming_filter_description():
    """Фильтр по подстроке в назначении платежа."""
    with patch("mcp_server_tochka_bank.server.TochkaAPI") as MockAPI:
        instance = MockAPI.return_value
        instance.get_first_account.return_value = MOCK_ACCOUNT
        instance.init_statement.return_value = "stmt-123"
        ...
        async with create_connected_server_and_client_session(mcp._mcp_server) as session:
            result = await session.call_tool("tochka_incoming", {
                "month": 3, "year": 2026, "description": "ТЕСТПЭЙ",
            })
            assert not result.isError
            data = json.loads(result.content[0].text)

            # Only 2 transactions have "ТЕСТПЭЙ" in description (tx 101, 102)
            # tx 103 has "Оплата по счёту" — excluded
            assert len(data["by_inn"]) == 2
            assert data["by_inn"]["770000000010"]["amount"] == 2917.33
            assert data["by_inn"]["770000000020"]["amount"] == 20000.0
            assert data["by_inn"]["770000000020"]["count"] == 1
            assert data["total_amount"] == 22917.33
            assert data["total_count"] == 2
